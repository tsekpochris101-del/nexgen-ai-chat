...
The repository includes `render.yaml` for Render's free web service. In Render, choose **New → Blueprint**, connect the GitHub repository, and Render will use the blueprint's build command (including development dependencies needed by Vite), health check, server settings, and pinned Node 20.19.5 runtime. Enter `GROQ_API_KEY`, `EXA_API_KEY`, and `ADMIN_EMAIL` when Render prompts for the values.
